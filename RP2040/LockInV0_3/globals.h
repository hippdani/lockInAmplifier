...
int64_t a[MAX_FILTER_OD+1]; //IIR filter coefficients (global)
int64_t b[MAX_FILTER_OD+1];
...
int64_t inputLimitInt(String inputMessage, int64_t lowerLimit, int64_t upperLimit)
{
  ...
  String inputValue = String(int(input));
  Serial.println(inputValue); //doesnt print 0s!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  clear_Serial_buff();
  return(input);
}
...
void changeStdParam(stateType returnState = WAITforINPUT)  //returns to returnState if successfull or WAITforINPUT if unsuccsessfull. standard returnState is also WAITforINPUT
{
  ...
  if(answ == 'n' || answ == 'N')
  {
    ...
    stdPostDS = inputLimitInt("Enter post filter downsampling factor (1-10000): ",1,10000);
    ...
    for(int i = 0; i < stdFilterOd; i++)
    {
      a[i] = inputLimitInt("enter filter coefficient a[" + String(i) +"] in Q3.15 format: ",0,1000000);  
      b[i] = inputLimitInt("enter filter coefficient b[" + String(i) +"] in Q3.15 format: ",0,1000000);  //(2<<18));  
    }
    if(a[0] != 32768) 
    {
      Serial.print("Warning: a[0] not eqval to 32768 (1 in Q3.15 format) -> this may lead to overflow and oscillation of the filter. a[0]="); Serial.print(a[0]);
    }
    saveStdParam();
    state = returnState;
