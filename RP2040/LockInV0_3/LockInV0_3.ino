...
//TODO: Fix inpuLimitInt not printing 0s
//TODO: Fix post ds saving. saving 200 sometimes stores 1300????
//TODO: Fix IIR filter
//TODO: Save std param and loadstdparam for a and b arrays.
    ...
    case LOCK_IN:
      while(true)
      {
        ...
        if(rp2040.fifo.pop_nb(&message))
        {
          if((message & 1) == 0) 
          {
            Xarray[NcounterX] = int32_t(message)/2; //remove lowest bit to make more headroom for filter calculation
            NcounterX++;
            if(NcounterX >= N) NcounterX = 0;
          }
          else
          {
            Yarray[NcounterY] = int32_t(message-1)/2; //-1 to Remove LSB that identifies X / Y, reform from uint to int. remove last bit -> 31 bit to make more headroom for filter calculation
          ...
          {
            if(stdFilterOd == 0) //order 0, moving average
            {
              Serial.println(); Serial.print(lowpass(Xarray, N, NcounterY)); Serial.print(", "); Serial.print(lowpass(Yarray, N, NcounterY));
            }
            else //IIR
            {
              Serial.println(); Serial.print(lowpassIIR(Xarray, NcounterY)); Serial.print(", "); Serial.print(lowpassIIR(Yarray, NcounterY));
            }
            postDScounter = 0;             
          }
        }
