...
float stdKI;
float stdKD;
unsigned int stdSetpoint;
int32_t a[8]; //IIR filter coefficients (global)
int32_t b[8];
bool fastCoreActive = 0;
...
bool setState(char newState) // sets the state variable according to some defined characters
{
  ...
  else if (newState == 'a') state = ENTER_MAN_APPR;
  ...
  else if (newState == 'p') state = CHANGE_PID;
...
{
  Serial.print("QTF Freq.: "); Serial.print(stdFreq); Serial.println(" Hz");
  Serial.print("KP: "); Serial.println(stdKP,5); 
  Serial.print("KI: "); Serial.println(stdKI,5); 
  Serial.print("KD: "); Serial.println(stdKD,5); 
  Serial.print("Setpoint: "); Serial.println(stdSetpoint); 
}
