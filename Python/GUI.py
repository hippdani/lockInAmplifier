...
class myGUI ():
    ...
    def __init__(self):
        ...
        self.connectFrame = tk.Frame(self.root)
        self.connectFrame.columnconfigure(0, weight=1)
        self.connectFrame.columnconfigure(1, weight=3)
        self.connectBtn = tk.Button(self.connectFrame, text="connect", font=('Arial', self.fontSize), command=self.connectEvent)
        self.connectBtn.grid(row=0, column=0, padx=40, pady=10, sticky=tk.W)
        self.connectionLabel = tk.Label(self.connectFrame, text=str(self.li.port)+" V"+str(self.li.firmware), font=('Arial',self.fontSize), bg='red')
        ...
        self.paramFrame = tk.Frame(self.root)
        ...
        self.intRadio = tk.Radiobutton(self.paramFrame, text='Int. Ref.', command=self.intRef, variable=self.radioValue, value='int',font=('Arial', self.fontSize))
        self.intRadio.grid(row=0, column=0, padx=10, pady=10, sticky=tk.W)
        self.extRadio = tk.Radiobutton(self.paramFrame, text='Ext. Ref.', command=self.extRef, variable=self.radioValue, value='ext',font=('Arial', self.fontSize))
        self.extRadio.grid(row=1, column=0, padx=10, pady=10, sticky=tk.W)
        #-----------------
        self.refFreqDivLabel = tk.Label(self.paramFrame, text="Divider", font=('Arial', self.fontSize)) #ext is standard, therfore divider is standard
        self.refFreqDivLabel.grid(row=0, column=1, padx=10, pady=10, sticky=tk.W)
        self.tauLabel = tk.Label(self.paramFrame, text="Timeconstant", font=('Arial', self.fontSize))
        self.tauLabel.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)
        self.refFreqDivEntry = tk.Entry(self.paramFrame, )
        self.refFreqDivEntry.grid(row=0, column=2, padx=10, pady=10, sticky=tk.W)
        self.tauEntry = tk.Entry(self.paramFrame, )
        ...
        self.refFreqDivDrop = tk.OptionMenu(self.paramFrame, self.refFreqDivDropValue, "mHz", "Hz", "kHz")
        self.refFreqDivDrop.config(state=tk.DISABLED)
        self.refFreqDivDrop.grid(row=0, column=3, padx=10, pady=10, sticky=tk.W)
        self.tauDropValue = tk.StringVar()
        self.tauDrop = tk.OptionMenu(self.paramFrame, self.tauDropValue, "ms", "s")
        self.tauDrop.grid(row=1, column=3, padx=10, pady=10, sticky=tk.W)
        #Filter order and type menu
        self.filterOdLabel = tk.Label(self.paramFrame, text="Filter Order", font=('Arial', self.fontSize))
        self.filterOdLabel.grid(row=0, column=4, padx=10, pady=10, sticky=tk.W)
        self.filterTypeLabel = tk.Label(self.paramFrame, text="Filter Type", font=('Arial', self.fontSize))
        ...
        self.filterOdDrop = tk.OptionMenu(self.paramFrame, self.filterOdDropValue, "1", "2", "3", "4", "6", "8")
        ...
        self.filterTypeDrop = tk.OptionMenu(self.paramFrame, self.filterTypeValue, "Bessel", "Butterworth", "Chebyshev")
        self.filterTypeDrop.grid(row=1, column=5, padx=10, pady=10, sticky=tk.W)
        self.paramFrame.pack(fill='x')
        self.samplesLabel = tk.Label(self.paramFrame, text="received samples / s", font=('Arial', self.fontSize))
        self.samplesLabel.grid(row=2, column=1, padx=10, pady=10, sticky=tk.W)
        self.samplesEntry = tk.Entry(self.paramFrame, )
        ...
        #Run menu
        self.runBtn = tk.Button(self.paramFrame, text='RUN', command=self.startLockIn)
        self.runBtn.grid(row = 3, column = 0, padx = 10, pady = 10, sticky = tk.W)
        self.plotTimeLabel = tk.Label(self.paramFrame, text="X-Axis: Time", font=('Arial', self.fontSize))
        self.plotTimeLabel.grid(row=3, column=1, padx=10, pady=10, sticky=tk.W)
        self.plotTimeEntry = tk.Entry(self.paramFrame, )
        ...
        self.saveBtn = tk.Button(self.paramFrame, text='save to', command=self.saveData)
        self.saveBtn.grid(row=3, column=3, padx=10, pady=10, sticky=tk.W)
        self.saveEntry = tk.Entry(self.paramFrame, )
        ...
        self.label2 = tk.Label(self.plotFrame, text="Graph Page!")
    ...
    def animate(self,i):
        if self.ani_is_paused == False:
            ...
            if len(newDataX) > 0 and len(newDataY) > 0:
                ...
                self.sbplt.set_ylim((np.min([self.xList,self.yList]),np.max([self.xList,self.yList])))
            #self.sbplt.clear()
    ...
    def connectEvent(self):
        ...
        if status:
            ...
            self.connectionLabel.config(bg='lightgreen')
        else:
            messagebox.showinfo(title="No working device found", message="Connecting unsucsessfull. "+ connectMsg)
            self.connectionLabel.config(bg='red')
        # update label
        self.connectionLabel.config(text=str(self.li.port)+" firmware V"+str(self.li.firmware))
    ...
    def startLockIn(self):
        ...
        if self.li.is_connected():
            ...
            self.runBtn.config(text='STOP', command=self.stopLockIn)
        else:
